...
tf.app.flags.DEFINE_string('train_log_dir','logs_v2', '''directory wherer to write event logs''')
...
def filequeue_to_batch_data(filename_queue, line_reader, batch_size = BATCH_SIZE):
    ...
    retrieve_tensor = uf.read_binary(retrieve_image_name, FEATURE_DIM)
...
def train():
    ...
    image_data_ph = tf.placeholder(tf.float32, shape = (2 * BATCH_SIZE, FEATURE_DIM))
