...
def gen_list(file_name_list):
    ...
    pos_index = range(len(image_d[0]))
    random.shuffle(pos_index)

    neg_image_list = list()
    for i in range(file_num)[1:]:
        neg_image_list += image_d[i]
    random.shuffle(neg_image_list)
    
    full_len = min(len(neg_image_list), len(image_d[0]))
    file_list = list()
    for i in range(full_len):
        curr_s = image_d[0][i] + " " + image_d[0][pos_index[i]] + \
            " " + "1" + "\n"
        curr_s = curr_s.replace(".jpg", ".cn5")
        file_list.append(curr_s)

        curr_s = image_d[0][i] + " " + neg_image_list[i] + \
            " " + "0" + "\n"
        curr_s = curr_s.replace(".jpg", ".cn5")
        file_list.append(curr_s)
...
if __name__ == "__main__":
    ...
    with open("file_list_train_v2_cn5.txt", "w") as f:
        for c in curr_list:
            f.write(c)
