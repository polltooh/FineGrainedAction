...
TRAIN_TXT = 'file_list_fine_tune_train_v2_nba.txt'
...
tf.app.flags.DEFINE_integer('max_training_iter', 500000,
        '''the max number of training iteration''')
tf.app.flags.DEFINE_float('init_learning_rate',0.1,
        '''initial learning rate''')
tf.app.flags.DEFINE_string('model_dir', 'fine_tune_nn_model_logs_v2','''directory where to save the model''')
...
def filequeue_to_batch_data(filename_queue, line_reader, batch_size = BATCH_SIZE):
    ...
    query_tensor = uf.read_binary(query_image_name, FEATURE_DIM)
...
def train():
    ...
    train_batch_feature, train_batch_label = filequeue_to_batch_data(train_filenamequeue, line_reader)

    train_batch_label_one_hot = dense_to_one_hot(train_batch_label, LABEL_DIM)

    global_step = tf.Variable(0, name = 'global_step', trainable = False)
    feature_ph = tf.placeholder(tf.float32, shape = (BATCH_SIZE, FEATURE_DIM))
    label_ph = tf.placeholder(tf.float32, shape = (BATCH_SIZE, LABEL_DIM))

    infer = fine_tune_nt.inference2(feature_ph, LABEL_DIM)
    ...
    if TRAIN:
        for i in xrange(FLAGS.max_training_iter):
            batch_feature_v, batch_label_v = sess.run([    
                train_batch_feature, train_batch_label_one_hot])

            feed_data = {feature_ph: batch_feature_v, label_ph: batch_label_v}
